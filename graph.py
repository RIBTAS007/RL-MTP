...
def Avg_Service_Urgency(cover,y):
    ...
    plt.grid(True)
    plt.plot( y,cover, label= "stars", color= "blue",marker=".") #linestyle='--'
    plt.savefig('Average Service Urgency for equal alphat.png')
    plt.close(fig)
...
def Avg_Loss(losses):  
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('Average_Loss_Valuest.png')
    plt.close(fal)
...
def Avg_Reward_Values(Mentrd):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('Average_Reward_Valuest.png')
    plt.close(far)
